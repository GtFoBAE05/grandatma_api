...
func CreatePersonalReservasi(c *gin.Context) {
	var reqBody models.PersonalReservasi
	...
	_, err = stmt.Exec(idReservasi, userID, reservasi.IdKamar, reservasi.TanggalCheckin, reservasi.TanggalCheckout,
	...
	query = `
	INSERT INTO jaminan (id_reservasi, nominal)
	VALUES ($1, $2)
	RETURNING id
	`
	...
	err = database.DBClient.QueryRow(query, idReservasi, 0).Scan(&idJaminan)
	...
	//add status menginap
	query = `
	INSERT INTO status_menginap (id_reservasi)
	VALUES ($1)
	`
	...
	//add transaksi
	query = `
	INSERT INTO transaksi (id_reservasi,tanggal_transaksi, total_pembayaran, id_deposit, 
		status_bayar, id_jaminan, created_at, updated_at)
	...
	_, err = stmt.Exec(idReservasi, time.Now(), totalPembayaran, idDeposit,
		false, idJaminan, reservasi.CreatedAt, reservasi.UpdatedAt)
